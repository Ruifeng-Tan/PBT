...
LLM_path = '/data/LLMs/models--meta-llama--Llama-3.1-70B-Instruct/snapshots/945c8663693130f8be2ee66210e062158b2a9693'
...
def read_cell_data_according_to_prefix(file_name, root_path):
    ...
    if prefix == 'MICH':
        with open(f'{root_path}/total_MICH_labels.json') as f:
            life_labels = json.load(f)
    elif prefix.startswith('Tongji'):
        with open(f'{root_path}/Tongji_labels.json') as f:
            life_labels = json.load(f)
    else:
        with open(f'{root_path}/{prefix}_labels.json') as f:
            life_labels = json.load(f)
...
def get_features_from_cellNames(cell_names):
    ...
    for cell_name in cell_names:
        ...
        # hidden_states = language_model(llama_enc_out, attention_mask=DLP_attention_mask).last_hidden_state
...
cb = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=colormap),label='Measured Cycle Life',ax=plt.gca())
plt.legend()
plt.xlim(-300, 250)
plt.ylim(-210, 300)
fig.tight_layout()

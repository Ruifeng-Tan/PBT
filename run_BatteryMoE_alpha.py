...
from models import BatteryMoE_DKPNorm, baseline_CPTransformerMoE, BatteryMoE_PESum_mask, BatteryMoE_PESum_mask_DKPNorm, baseline_CPMLPMoE
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_PESum_mask':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_PESum_mask.Model(model_config)
    elif args.model == 'BatteryMoE_DKPNorm':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_DKPNorm.Model(model_config)
    elif args.model == 'BatteryMoE_PESum_mask_DKPNorm':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_PESum_mask_DKPNorm.Model(model_config)

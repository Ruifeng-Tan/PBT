...
from models import BatteryMoE_horizontal_MHv2, baseline_CPTransformerMoE, BatteryMoE_Sparse_noAnode, BatteryMoE_Sparse_flexible, BatteryMoE_Sparse
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_Sparse_noAnode':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_Sparse_noAnode.Model(model_config)
    elif args.model == 'BatteryMoE_horizontal_MHv2':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_horizontal_MHv2.Model(model_config)
    elif args.model == 'BatteryMoE_Sparse_flexible':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)

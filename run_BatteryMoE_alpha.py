...
from models import BatteryMoE_Hyper_CropAugIMP45, BatteryMoE_Hyper_RMS, baseline_CPTransformerMoE, BatteryMoE_Hyper_CropAugIMP, baseline_CPMLPMoE
import pickle
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_Hyper_CropAugIMP45':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_Hyper_CropAugIMP45.Model(model_config)

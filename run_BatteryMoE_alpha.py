...
from models import BatteryMoE_DKPNorm_imp, baseline_CPTransformerMoE, BatteryMoE, BatteryMoE_bottleneck, BatteryMoE_DKPNorm_asym
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_DKPNorm_imp':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_DKPNorm_imp.Model(model_config)
    elif args.model == 'BatteryMoE_bottleneck':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_bottleneck.Model(model_config)
    elif args.model == 'BatteryMoE_DKPNorm_asym':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_DKPNorm_asym.Model(model_config)
    else:

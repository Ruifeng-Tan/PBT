...
from models import BatteryMoE_DownAug, BatteryMoE_Hyper_CropAugIMPR2, baseline_CPTransformerMoE, BatteryMoE_Hyper_CropAugIMP, baseline_CPMLPMoE
import pickle
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_DownAug':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_DownAug.Model(model_config)

...
class Dataset_BatteryLifeLLM_original(Dataset):
    ...
    def get_charge_discharge_curves(self, file_name, df, early_cycle_threshold, nominal_capacity):
        ...
        for cycle in range(1, early_cycle_threshold+1):
            if cycle in df['cycle_number'].unique():
                ...
                # cutoff_voltage_charge = np.max(voltage_records)
                # cutoff_voltage_indices = np.nonzero(voltage_records==cutoff_voltage_charge) # This includes constant-voltage charge data
                ...
                # cutoff_voltage_discharge = np.min(voltage_records)
                # cutoff_voltage_indices = np.nonzero(voltage_records==cutoff_voltage_discharge) 
    ...
    def resample_charge_discharge_curvesv2(self, voltages, currents, capacity_in_battery):
        '''
        resample the charge and discharge curves based on the capacity
        :param voltages:charge or dicharge voltages
        :param currents: charge or discharge current
        :param capacity_in_battery: remaining capacities in the battery
        :return:interploted records
        '''
        def remove_non_increasing_points(x):
            # Start with the first element, and add elements that are strictly increasing
            filtered_x = [x[0]]
            filtered_indices = [0]
            for i in range(1, len(x)):
                if x[i] > filtered_x[-1]:  # Check if the current point is strictly greater than the last added point
                    filtered_x.append(x[i])
                    filtered_indices.append(i)
            return filtered_x, filtered_indices
        
        def is_strictly_increasing(x):
            for i in range(1, len(x)):
                if x[i] <= x[i - 1]:  # If any value is not strictly greater than the previous one
                    return False
            return True
        
        charge_discharge_len = self.charge_discharge_len // 2
        while not is_strictly_increasing(capacity_in_battery):
            # remove some flucuating capacity points that are not strictly increasing
            capacity_in_battery, filtered_indices = remove_non_increasing_points(capacity_in_battery)
            voltages = voltages[filtered_indices]
            currents = currents[filtered_indices]
        interp_capacity_in_battery = np.linspace(min(capacity_in_battery), max(capacity_in_battery), num=charge_discharge_len,
                                        endpoint=True)
        interp_voltages = np.interp(interp_capacity_in_battery, capacity_in_battery, voltages)
        interp_currents = np.interp(interp_capacity_in_battery, capacity_in_battery, currents)
        return interp_voltages, interp_currents, interp_capacity_in_battery

    def generate_monotonic_sequence(self, length, start, end):
        ...
        random_increments = np.random.rand(length)

        # Cumulative sum to get a monotonically increasing sequence
        sequence = np.cumsum(random_increments)

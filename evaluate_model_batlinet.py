...
import evaluate
from utils.tools import get_support_set
...
from data_provider.data_factory import data_provider_LLMv2, data_provider_LLM_evaluate, data_provider_evaluate_BL
import time
import random
import numpy as np
import os
...
parser.add_argument('--finetune_method', type=str, default=None, help='FT, AT, or AT_nCP; kept for compatibility with evaluation shell scripts')
args = parser.parse_args()
eval_cycle_min = args.eval_cycle_min
eval_cycle_max = args.eval_cycle_max
batch_size = args.batch_size
cli_finetune_method = args.finetune_method
...
if 'finetune_method' not in args_json and cli_finetune_method is not None:
    args_json['finetune_method'] = cli_finetune_method

# BatLiNet's baseline data loader may use target_dataset for val/test splits.
# Setting it only for BatLiNet keeps the original evaluation behavior unchanged for all other models.
...
for ii in range(args.itr):
    ...
    is_batlinet = args.model == 'BatLiNet'

    # Keep the original PBT/CP* evaluation path untouched.  BatLiNet-specific
    # modules are imported lazily so they cannot affect evaluation of other models.
    if is_batlinet:
    ...
    if (not is_batlinet) and finetune_method == 'AT':
    ...
    elif (not is_batlinet) and finetune_method == 'AT_nCP':
    ...
    label_scaler = joblib.load(os.path.join(path, 'label_scaler'))
    std, mean_value = np.sqrt(label_scaler.var_[-1]), label_scaler.mean_[-1]
    accelerator.print("Loading test samples......")

    if is_batlinet:
        # The BatLiNet training script saves both scalers and uses
        # data_factory_with_BLN.data_provider_baseline.
        ...
        test_data, test_loader = data_provider_func(args, 'test', label_scaler=label_scaler, eval_cycle_min=eval_cycle_min, eval_cycle_max=eval_cycle_max, life_class_scaler=life_class_scaler)
    else:
        ...
        test_data, test_loader = data_provider_func(
            ...
            eval_cycle_max=eval_cycle_max, temperature2mask=temperature2mask,
            format2mask=format2mask, cathodes2mask=cathodes2mask,
            anode2mask=anode2mask, ion2mask=ion2mask,
    ...
    with torch.no_grad():
        if is_batlinet:
            for i, batch in tqdm(enumerate(test_loader)):
                # BatLiNet's collate function used by the training script returns:
                # cycle_curve_data, curve_attn_mask, labels, life_class,
                # scaled_life_class, weights, seen_unseen_ids, features, data_batch
                if len(batch) != 9:
                    raise RuntimeError(
                        f'BatLiNet expects a 9-item batch from data_factory_with_BLN, but got {len(batch)} items.'
                    )

                (cycle_curve_data, curve_attn_mask, labels, life_class, scaled_life_class,
                 weights, seen_unseen_ids, features, data_batch) = batch

                seen_number_of_cycles = torch.sum(curve_attn_mask, dim=1)

                # This is exactly the feature/support-set route used during BatLiNet training.
                ...
                sup_x, sup_y = get_support_set(
                    raw_x,
                    test_data.total_features,
                    test_data.total_labels,
                    args,
                    training=False
                )
                ...
                seen_unseen_ids = seen_unseen_ids.to(accelerator.device)
                seen_number_of_cycles = seen_number_of_cycles.to(accelerator.device)

                batlinet_result = model(x, y, sup_x, sup_y, training=False)
                # Be compatible with both `outputs` and `(outputs, loss)` implementations.
                outputs = batlinet_result[0] if isinstance(batlinet_result, (tuple, list)) else batlinet_result
                ...
                # Dataset/domain IDs are not explicit in the BatLiNet training batch.
                # Reuse them from data_batch when available; otherwise use a single-domain
                # placeholder. This does not affect MAPE/alpha-accuracy/seen-unseen metrics.
                dataset_ids = getattr(data_batch, 'dataset_ids', None)
                if dataset_ids is None:
                    dataset_ids = getattr(data_batch, 'dataset_id', None)
                domain_ids = getattr(data_batch, 'domain_ids', None)
                if domain_ids is None:
                    domain_ids = getattr(data_batch, 'domain_id', None)

                if dataset_ids is None:
                    dataset_ids = torch.zeros(cut_off, device=accelerator.device)
                elif not torch.is_tensor(dataset_ids):
                    dataset_ids = torch.as_tensor(dataset_ids, device=accelerator.device)
                else:
                    dataset_ids = dataset_ids.to(accelerator.device)
                dataset_ids = dataset_ids.reshape(-1)[:cut_off]
                if dataset_ids.numel() == 1 and cut_off > 1:
                    dataset_ids = dataset_ids.repeat(cut_off)

                if domain_ids is None:
                    domain_ids = dataset_ids.clone()
                elif not torch.is_tensor(domain_ids):
                    domain_ids = torch.as_tensor(domain_ids, device=accelerator.device)
                else:
                    domain_ids = domain_ids.to(accelerator.device)
                domain_ids = domain_ids.reshape(-1)[:cut_off]
                if domain_ids.numel() == 1 and cut_off > 1:
                    domain_ids = domain_ids.repeat(cut_off)

                (all_predictions, all_targets, dataset_ids, seen_unseen_ids,
                 domain_ids, seen_number_of_cycles) = accelerator.gather_for_metrics((
                    transformed_preds, transformed_labels, dataset_ids, seen_unseen_ids,
                    domain_ids, seen_number_of_cycles
                ))

                total_preds += all_predictions.detach().cpu().numpy().reshape(-1).tolist()
                total_domain_ids += domain_ids.detach().cpu().numpy().reshape(-1).tolist()
                total_references += all_targets.detach().cpu().numpy().reshape(-1).tolist()
                total_dataset_ids += dataset_ids.detach().cpu().numpy().reshape(-1).tolist()
                total_seen_unseen_ids += seen_unseen_ids.detach().cpu().numpy().reshape(-1).tolist()
                total_seen_number_of_cycles += seen_number_of_cycles.detach().cpu().numpy().reshape(-1).tolist()
        else:
            # Original evaluation path for PBT / CPTransformerDeepSeekMoE / CPMLP / CPTransformer.

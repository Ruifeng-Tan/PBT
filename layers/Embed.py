...
class PositionalEmbedding_BL(nn.Module):
    def __init__(self, d_model, max_len=30000):
        super(PositionalEmbedding_BL, self).__init__()
        # Compute the positional encodings once in log space.
        pe = torch.zeros(max_len, d_model).float()
        pe.require_grad = False
...
class BatteryDataEmbedding(nn.Module):
    def __init__(self, c_in, fixed_len, d_model, embed_type='fixed', freq='h', dropout=0.1):
        super(BatteryDataEmbedding, self).__init__()

        # local embedding via CNN
        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model)
        self.embed_type = embed_type
        self.dropout = nn.Dropout(p=dropout)

        # global embedding via MLP
        self.mlp = nn.Sequential(nn.Flatten(start_dim=-2), nn.Linear(fixed_len*c_in, d_model))

    def forward(self, x, x_mark):
        '''
            x: [B*L, fixed_len, 3]
        '''
        local_x = self.value_embedding(x) # [B*L, fixed_len_ d_model]
        global_x = self.mlp(x).unsqueeze(1) # [B*L, 1, d_model]
        x = torch.cat([global_x, local_x], dim=1) # [B*L, fixed_len+1, d_model]
        return self.dropout(x)

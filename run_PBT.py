...
from models import PBT, baseline_CPTransformerMoE, baseline_CPMLPMoE, CPTransformer_ablation, PBT_woIE2
import pickle
import wandb
...
parser.add_argument('--min_d_ff', type=int, default=32, help='the min dimension of fcn')
...
for ii in range(args.itr):
    ...
    elif args.model == 'PBT_woIE2':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = PBT_woIE2.Model(model_config)

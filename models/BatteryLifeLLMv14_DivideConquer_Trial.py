'''
v14 DvideConquer uses the HiMLP as the backbone
'''
import torch
...
class BatteryExpert(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.intra_MLP = nn.ModuleList([MLPBlock(self.d_model, self.d_ff, self.d_model, self.drop_rate, activation=configs.activation) for _ in range(configs.e_layers)])
        
        self.DKP_MLP = nn.Sequential(nn.Linear(self.d_llm, self.d_ff), nn.ReLU(),
                                     nn.Linear(self.d_ff, self.d_model))
        self.flatten_head = nn.Sequential(nn.Flatten(start_dim=1), nn.Linear(self.d_model*(configs.early_cycle_threshold+1), self.d_model))
        self.inter_MLP = nn.ModuleList([MLPBlock(self.d_model, self.d_ff, self.d_model, self.drop_rate, activation=configs.activation) for _ in range(configs.d_layers)])
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, DKP_embeddings):
        ...
        DKP_embeddings = self.DKP_MLP(DKP_embeddings)
        whole_cycle_data = torch.cat([DKP_embeddings, cycle_curve_data], dim=1) # [B, L+1, d_model]
        whole_cycle_data = self.flatten_head(whole_cycle_data) # [B, d_model]
        for i in range(self.d_layers):
            whole_cycle_data = self.inter_MLP[i](whole_cycle_data) # [B, L, d_model]
...
class Model(BatteryLifeLLM):
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, input_ids: torch.LongTensor = None,
        ...
        logits = logits.masked_fill_(mask==0, -np.inf)
        logits = F.softmax(logits, dim=1)
        # Set all values that are not top-K to zero

        # logits = logits.to(torch.bfloat16) # [B, num_experts]
        dispatcher = MOEDispatcher(self.num_experts, logits)
        MOE_indicies = dispatcher.dispatch()
        total_dec_outs = []
        for i, expert in enumerate(self.experts):
            dec_out = expert(cycle_curve_data[MOE_indicies[i]], curve_attn_mask[MOE_indicies[i]], DKP_embeddings[MOE_indicies[i]]) # [expert_batch_size, d_llm]
            total_dec_outs.append(dec_out)
        total_dec_outs = dispatcher.combine(total_dec_outs).to(torch.bfloat16) # [B, d_model]
        preds, llm_out, feature_llm_out = self.regression_head(total_dec_outs, attention_mask)
        ...
        return preds, None, llm_out, feature_llm_out, label_preds, label_feature_llm_out, logits.float()

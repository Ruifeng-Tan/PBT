...
class IntraThreeMoELayer(nn.Module):
    def __init__(self, configs):
        super(IntraThreeMoELayer, self).__init__()
        ...
        self.cathodeMoE = HEIntraCycleMoELayer(configs, self.cathode_experts)
        self.temperatureMoE = HEIntraCycleMoELayer(configs, self.temperature_experts)
        self.formatMoE = HEIntraCycleMoELayer(configs, self.format_experts)

    def forward(self, x, logits, cathode_masks, temperature_masks, format_masks):
        total_guide_loss = 0
        total_aug_count = 0

        out, _, guide_loss = self.cathodeMoE(x, logits[:,:self.cathode_split], cathode_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1

        out, _, guide_loss = self.temperatureMoE(out, logits[:,self.cathode_split:self.temperature_experts_split], temperature_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1

        out, _, guide_loss = self.formatMoE(out, logits[:,self.temperature_experts_split:], format_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1 

        return out, total_guide_loss, total_aug_count

class InterThreeMoELayer(nn.Module):
    def __init__(self, configs):
        super(InterThreeMoELayer, self).__init__()
        self.cathode_experts = configs.cathode_experts
        self.temperature_experts = configs.temperature_experts
        self.format_experts = configs.format_experts
        self.cathode_split = self.cathode_experts
        self.temperature_experts_split = self.cathode_experts+self.temperature_experts
        self.cathodeMoE = HEInterCycleMoELayer(configs, self.cathode_experts)
        self.temperatureMoE = HEInterCycleMoELayer(configs, self.temperature_experts)
        self.formatMoE = HEInterCycleMoELayer(configs, self.format_experts)
...
class HEIntraCycleMoELayer(nn.Module):
    ...
    def forward(self, cycle_curve_data, logits, HE_mask):
        '''
        params:
            cycle_curve_data: [B, L, d_model]
            logits: [B, num_experts]
            HE_mask: [B, num_experts]
        '''
        B = cycle_curve_data.shape[0]


        mask = torch.where(HE_mask==1, torch.ones_like(logits), torch.zeros_like(logits))
        logits = F.softmax(logits, dim=1) # [B, num_experts]
        raw_logits = logits.clone()
        # logits.masked_fill_(mask==0, 0) # [B, num_experts]
        logits = logits * mask
...
class HEInterCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts):
        super(HEInterCycleMoELayer, self).__init__()
        self.charge_discharge_length = configs.charge_discharge_length # There two summary tokens
        self.drop_rate = configs.dropout
        self.n_heads = configs.n_heads
        self.d_ff = configs.d_ff
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.cathode_split = self.cathode_experts
        self.temperature_experts_split = self.cathode_experts+self.temperature_experts
        self.gate = nn.Sequential(nn.Linear(self.d_llm, (self.cathode_experts + self.temperature_experts + self.format_experts)*(2+self.e_layers+self.d_layers)))
        ...
        self.intra_MoE_layers = nn.ModuleList([IntraThreeMoELayer(configs) for _ in range(self.e_layers)])
        ...
        self.inter_MoE_layers = nn.ModuleList([InterThreeMoELayer(configs) for _ in range(self.d_layers)])
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        out, _, guide_loss = self.flattenIntraCycleLayer(cycle_curve_data, logits[:,logits_index,:self.cathode_split], cathode_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        ...
        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss, aug_count = intra_MoELayer(out, logits[:,logits_index], cathode_masks, temperature_masks, format_masks)
            logits_index += 1
            total_guide_loss += guide_loss

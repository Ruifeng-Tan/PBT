'''
v21就是在v20的基础上，把DKP也作为输入加在了FlattenInterCycleMoELayer
'''
import torch
...
class FlattenInterCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.activation = configs.activation
        self.top_k = configs.topK
        self.DKP_experts = nn.ModuleList([nn.Sequential(nn.Linear(self.d_llm, self.d_ff), nn.ReLU(), nn.Linear(self.d_ff, self.d_model)) for _ in range(self.num_experts)])
        self.general_DKP_expert = nn.Sequential(nn.Linear(self.d_llm, self.d_ff), nn.ReLU(), nn.Linear(self.d_ff, self.d_model))
    ...
    def forward(self, cycle_curve_data, DKP_embeddings):
        ...
        total_outs = []
        for i, expert in enumerate(self.experts):
            DKP_out = self.DKP_experts[i](DKP_embeddings[MOE_indicies[i]]).unsqueeze(1) # [expert_batch_size, 1, d_model]
            cylce_data_input = cycle_curve_data[MOE_indicies[i]]  # [expert_batch_size, L, d_model]
            out = torch.cat([DKP_out, cylce_data_input], dim=1)
            out = expert(out) # [expert_batch_size, d_llm]
            total_outs.append(out)

        total_outs = dispatcher.combine(total_outs).to(torch.bfloat16) # [B, d_model]
        DKP_out = self.general_DKP_expert(DKP_embeddings).unsqueeze(1)
        out = torch.cat([DKP_out, cycle_curve_data], dim=1)
        final_out = self.general_expert(out) + total_outs

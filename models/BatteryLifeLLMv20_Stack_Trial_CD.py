...
class FlattenIntraCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.experts = nn.ModuleList([nn.Sequential(nn.Flatten(start_dim=2), nn.Linear(self.charge_discharge_length*3//2, self.d_model)) for _ in range(self.num_experts)])
        self.general_expert = nn.Sequential(nn.Flatten(start_dim=2), nn.Linear(self.charge_discharge_length*3//2, self.d_model))
    ...
    def forward(self, cycle_curve_data, DKP_embeddings):
        ...
        charge_cycle_curve_data = cycle_curve_data[:,:,:,:self.charge_discharge_length//2]
        discharge_cycle_curve_data = cycle_curve_data[:,:,:,self.charge_discharge_length//2:]
        cycle_curve_data = torch.cat([charge_cycle_curve_data, discharge_cycle_curve_data], dim=1) # [B, 2*L, 3, fixed_len//2]
...
class FlattenInterCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.activation = configs.activation
        self.top_k = configs.topK
        self.experts = nn.ModuleList([nn.Sequential(nn.Flatten(start_dim=1), nn.Linear(self.early_cycle_threshold*self.d_model*2, self.d_model)) for _ in range(self.num_experts)])
        self.general_expert = nn.Sequential(nn.Flatten(start_dim=1), nn.Linear(self.early_cycle_threshold*self.d_model*2, self.d_model))
        self.noisy_gating = configs.noisy_gating
        self.gate  = PatternRouter(self.d_llm, self.num_experts)
        self.noise = PatternRouter(self.d_llm, self.num_experts)
        self.softplus = nn.Softplus()

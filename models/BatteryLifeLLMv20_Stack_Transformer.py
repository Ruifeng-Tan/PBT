...
class BatteryMoEEncoderLayers(nn.Module):
    def __init__(self, configs):
        super(BatteryMoEEncoderLayers, self).__init__()
        self.charge_discharge_length = configs.charge_discharge_length # There two summary tokens
        self.drop_rate = configs.dropout
        self.n_heads = configs.n_heads
        self.d_ff = configs.d_ff
        ...
        self.top_k = configs.topK
        self.attention = AttentionLayer(FullAttention(True, attention_dropout=0.1),
                                         self.d_model, n_heads=self.n_heads)
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, DKP_embeddings):
        ...
        cycle_curve_data, _ = self.attention(cycle_curve_data, cycle_curve_data, cycle_curve_data, curve_attn_mask)
...
class Model(BatteryLifeLLM):
    def __init__(self, battery_life_config):
        ...
        self.batteryMoEEncoderLayers = nn.ModuleList([BatteryMoEEncoderLayers(configs) for _ in range(configs.e_layers)])
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, input_ids: torch.LongTensor = None,
        ...
        for i, layer in enumerate(self.batteryMoEEncoderLayers):
            out, aug_loss = layer(out, curve_attn_mask, DKP_embeddings)
            total_aug_loss += aug_loss
            total_aug_count += 1

...
class MultiViewLayer(nn.Module):
    ...
    def forward(self, x, total_logits, total_masks, view_logits):
        '''
        x: [N, *, in_dim]
        total_logits: [num_view, N, num_experts for each view expert]
        total_masks: [num_view, N, num_experts for each view expert]
        view_logits: [N, num_view]
        '''
        ...
        for i, view_expert in enumerate(self.view_experts):
            ...
            if len(out.size()) == 3:
                final_out = final_out + view_logits[:,i].reshape(-1, 1, 1) * out
            else:
                final_out = final_out + view_logits[:,i].reshape(-1, 1) * out
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.view_gate = nn.Sequential(nn.Linear(self.d_llm, self.num_views*(2+self.moe_layers)))
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        view_logits = self.view_gate(DKP_embeddings) # [N, num_view*moe_layers]
        view_logits = view_logits.reshape(B, -1 , self.num_views)
        view_logits = F.softmax(view_logits, dim=-1)
        ...
        out, guide_loss = self.flattenIntraCycleLayer(cycle_curve_data, [cathode_logits[:,logits_index],anode_logits[:,logits_index],temperature_logits[:,logits_index],format_logits[:,logits_index]], total_masks, view_logits[:, logits_index]) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        logits_index += 1

        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss = intra_MoELayer(out, [cathode_logits[:,logits_index],anode_logits[:,logits_index],temperature_logits[:,logits_index],format_logits[:,logits_index]], total_masks, view_logits[:, logits_index]) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1

        # out = out.reshape(B, -1) # flatten
        out, guide_loss = self.flattenInterCycleLayer(out, [cathode_logits[:,logits_index],anode_logits[:,logits_index],temperature_logits[:,logits_index],format_logits[:,logits_index]], total_masks, view_logits[:, logits_index])
        total_guide_loss += guide_loss
        total_aug_count += 1
        logits_index += 1

        for i, inter_MoELayer in enumerate(self.inter_MoE_layers):
            out, guide_loss = inter_MoELayer(out, [cathode_logits[:,logits_index],anode_logits[:,logits_index],temperature_logits[:,logits_index],format_logits[:,logits_index]], total_masks, view_logits[:, logits_index])
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1

'''
基于BatteryMoE_Hyper，只不过最后一层也使用MoE
'''
import torch
...
class BatteryMoEOutputLayer(nn.Module):
    def __init__(self, configs, num_experts):
        super(BatteryMoEOutputLayer, self).__init__()
        self.charge_discharge_length = configs.charge_discharge_length # There two summary tokens
        self.drop_rate = configs.dropout
        self.n_heads = configs.n_heads
        ...
        self.num_experts = num_experts 
        self.activation = configs.activation
        self.experts = nn.ModuleList([nn.Linear(self.d_model, configs.output_num) for _ in range(self.num_experts)])
        self.num_general_experts = configs.num_general_experts
        self.eps = 1e-9
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.output_hyperMoE = nn.ModuleList([HyperMoE(self.d_model, self.low_d_ff, configs.output_num) for _ in range(configs.num_hyper_experts)])
        ...
        # self.regression_head = OutputHead(configs)
        self.regression_head = MultiViewLayer(self.output_hyperMoE, self.low_d_ff,
                                                     nn.ModuleList([BatteryMoEOutputLayer(configs, self.num_experts),
                                                    ]), 
                                                    norm_layer=nn.LayerNorm(self.d_model),
                                                    general_experts=nn.ModuleList([
                                                        nn.Linear(self.d_model, configs.output_num) for _ in range(self.num_general_experts)
                                                    ]),
                                                    use_connection = False
                                                    )
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        preds, guide_loss  = self.regression_head(out, [logits[:,logits_index]], total_masks, selection_embeddings=selection_embeddings)

        preds = preds.float()
        llm_out = feature_llm_out = out.float()

...
class MultiViewTransformerLayer(nn.Module):
    ...
    def forward(self, x, total_logits, total_masks, attn_mask, selection_embeddings, DKP_hpyer_input):
        ...
        attn_mask: [B, 1, L, L]
        selection_embeddings: [B, num_experts, low_d_ff // 2]
        DKP_hpyer_input: [DKP_hpyer_input, low_d_ff]
        '''
        B = x.shape[0]
        layer_embedding = self.layer_embedding.expand(B, -1)
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.DKP_MLP = nn.Sequential(nn.Linear(self.d_llm, self.d_ff), nn.ReLU(), nn.Linear(self.d_ff, self.low_d_ff*self.moe_layers))
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        DKP_hpyer_input = DKP_hpyer_input.reshape(B, self.moe_layers, -1)
        ...
        out, guide_loss = self.flattenIntraCycleLayer(cycle_curve_data, [logits[:,logits_index]], total_masks, selection_embeddings=selection_embeddings, DKP_hpyer_input=DKP_hpyer_input[:, logits_index]) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        logits_index += 1

        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss = intra_MoELayer(out, [logits[:,logits_index]], total_masks, selection_embeddings=selection_embeddings, DKP_hpyer_input=DKP_hpyer_input[:, logits_index]) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1
        ...
        for i, inter_MoELayer in enumerate(self.inter_MoE_layers):
            out, guide_loss = inter_MoELayer(out, [logits[:,logits_index]], total_masks, attn_mask=attn_mask, selection_embeddings=selection_embeddings, DKP_hpyer_input=DKP_hpyer_input[:, logits_index]) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1

'''
router只有flatten layer使用DKP和cycle number
其余层的router使用embedding本身
'''
import torch
...
from layers.distributional_router_encoder import PatternRouter_two, PatternRouter_three, PatternRouter
...
class IntraCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.noisy_gating = configs.noisy_gating
        self.gate  = PatternRouter(self.d_model*self.early_cycle_threshold, self.num_experts)
        self.noise = PatternRouter(self.d_model*self.early_cycle_threshold, self.num_experts)
        self.softplus = nn.Softplus()
        self.eps = 1e-9
    ...
    def forward(self, cycle_curve_data, cycle_numbers, DKP_embeddings):
        ...
        clean_logits = self.gate(cycle_curve_data.reshape(B, -1))
        if self.noisy_gating and self.training:
            raw_noise_stddev = self.noise(cycle_curve_data.reshape(B, -1))
            noise_stddev = ((self.softplus(raw_noise_stddev) + 1e-2))
            noise = torch.randn_like(clean_logits)
            noisy_logits = clean_logits + (noise * noise_stddev)
            logits = noisy_logits
...
class FlattenInterCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.DKP_experts = nn.ModuleList([nn.Linear(self.d_llm, self.d_model) for _ in range(self.num_experts)])
        self.experts = nn.ModuleList([nn.Sequential(nn.Flatten(start_dim=1), nn.Linear((self.early_cycle_threshold+1)*self.d_model, self.d_model)) for _ in range(self.num_experts)])
        self.general_DKP_expert = nn.Linear(self.d_llm, self.d_model)
        self.general_expert = nn.Sequential(nn.Flatten(start_dim=1), nn.Linear((self.early_cycle_threshold+1)*self.d_model, self.d_model))
        self.noisy_gating = configs.noisy_gating
        self.gate  = PatternRouter_two(self.d_llm, 1, self.d_ff, self.num_experts)
        self.noise = PatternRouter_two(self.d_llm, 1, self.d_ff, self.num_experts)
        self.softplus = nn.Softplus()
    ...
    def forward(self, cycle_curve_data, cycle_numbers, DKP_embeddings):
        ...
        total_outs = []
        for i, expert in enumerate(self.experts):
            DKP_out = self.DKP_experts[i](DKP_embeddings[MOE_indicies[i]])
            DKP_out = DKP_out.unsqueeze(1) # [B, 1, d_model]
            selected_cycle_curve_data = cycle_curve_data[MOE_indicies[i]]
            tmp = torch.cat([DKP_out, selected_cycle_curve_data], dim=1)
            out = expert(tmp) # [expert_batch_size, d_model]
            total_outs.append(out)

        total_outs = dispatcher.combine(total_outs).to(torch.bfloat16) # [B, L, d_model]
        DKP_out = self.general_DKP_expert(DKP_embeddings)
        DKP_out = DKP_out.unsqueeze(1)
        cycle_curve_data = torch.cat([DKP_out, cycle_curve_data], dim=1)
        final_out = self.general_expert(cycle_curve_data) + total_outs
...
class InterCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.general_expert = MLPBlock(self.d_model, self.d_ff, self.drop_rate, self.activation)
        self.noisy_gating = configs.noisy_gating
        self.gate  = PatternRouter(self.d_model, self.num_experts)
        self.noise = PatternRouter(self.d_model, self.num_experts)
        self.softplus = nn.Softplus()
        self.eps = 1e-9
    ...
    def forward(self, cycle_curve_data, cycle_numbers, DKP_embeddings):
        ...
        '''
        B = cycle_curve_data.shape[0]
        clean_logits = self.gate(cycle_curve_data)
        if self.noisy_gating and self.training:
            raw_noise_stddev = self.noise(cycle_curve_data)
            noise_stddev = ((self.softplus(raw_noise_stddev) + 1e-2))
            noise = torch.randn_like(clean_logits)
            noisy_logits = clean_logits + (noise * noise_stddev)
            logits = noisy_logits

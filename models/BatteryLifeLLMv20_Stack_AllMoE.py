...
此外，outputhead使用linea layer，并且由top-1 gating控制
'''
import torch
...
class OutputHead(nn.Module):
    def __init__(self, ec_config):
        ...
        self.num_experts = ec_config.num_experts
        self.top_k = ec_config.topK
        self.gate  = PatternRouter(self.d_model, self.num_experts)
        self.experts = nn.ModuleList([nn.Linear(self.d_model, ec_config.output_num) for _ in range(self.num_experts)])
        self.general_expert = nn.Sequential(nn.Linear(self.d_model, ec_config.output_num))
        self.eps = 1e-9
    ...
    def forward(self, llm_out, attn_mask):
        ...
        total_outs = []
        for i, expert in enumerate(self.experts):
            out = expert(llm_out[MOE_indicies[i]]) # [expert_batch_size, d_llm]
            total_outs.append(out)

        total_outs = dispatcher.combine(total_outs).to(torch.bfloat16) # [B, L, d_model]
        final_out = self.general_expert(llm_out) + total_outs
        return final_out, llm_out, llm_out

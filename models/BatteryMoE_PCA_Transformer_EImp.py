'''
基于BatteryMoE_PCA_Transformer_first_Imp，只不过对Expert network structure进行了改进
'''
import torch
...
class MultiViewLayer(nn.Module):
    def __init__(self, in_dim, d_model, specification_view_experts, operating_view_experts, norm_layer, general_experts, use_connection):
        super(MultiViewLayer, self).__init__()
        self.num_views = len(specification_view_experts) + len(operating_view_experts)
        self.d_model = d_model
        self.in_dim = in_dim # input dimension
        self.num_s_views = len(specification_view_experts)

        self.cathode_anode_info_spliter = nn.Linear(self.in_dim, self.d_model*2)
        self.cathode_anode_info_merger = nn.Linear(self.d_model*2, self.d_model)
        
        self.s_view_experts = specification_view_experts
        self.o_view_experts = operating_view_experts
    ...
    def forward(self, x, total_logits, total_masks):
        ...
        # split the token embeddings into cathode and anode part
        # process the information from the cathode and anode views
        total_outs = []
        cathode_anode_x = self.cathode_anode_info_spliter(x)
        for i, view_expert in enumerate(self.s_view_experts):
            if cathode_anode_x.dim() == 3:
                s_out, guide_loss = view_expert(cathode_anode_x[:, :, self.d_model*i:self.d_model*(i+1)], total_logits[i], total_masks[i])
            elif cathode_anode_x.dim() == 2:
                s_out, guide_loss = view_expert(cathode_anode_x[:, self.d_model*i:self.d_model*(i+1)], total_logits[i], total_masks[i])
            else:
                raise Exception('Unexpected dimension of input!')
            total_guide_loss += guide_loss

            total_outs.append(s_out)
        
        s_out = torch.cat(total_outs, dim=-1)
        s_out = self.cathode_anode_info_merger(s_out) # [N, *, d_model]
        # process the information from the operating condition view
        for i, view_expert in enumerate(self.o_view_experts):
            out, guide_loss = view_expert(s_out, total_logits[i+self.num_s_views], total_masks[i+self.num_s_views])
            final_out = final_out + out
            total_guide_loss += guide_loss

        # process the informaiton using general experts
...
class MultiViewTransformerLayer(nn.Module):
    def __init__(self, in_dim, d_model, n_heads, specification_view_experts, operating_view_experts, general_experts, drop_rate):
...
class BatteryMoEFlattenIntraCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts):
        ...
        # self.top_k = 2
        self.experts = nn.ModuleList([nn.Linear(self.d_model, self.d_model) for _ in range(self.num_experts)])
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.flattn = nn.Flatten(start_dim=2)
        self.flattenIntraCycleLayer = MultiViewLayer(self.charge_discharge_length*3, self.d_model,
                                                    specification_view_experts=nn.ModuleList([BatteryMoEFlattenIntraCycleMoELayer(configs, self.cathode_experts),
                                                                                              BatteryMoEFlattenIntraCycleMoELayer(configs, self.anode_experts)]),
                                                    operating_view_experts=nn.ModuleList([BatteryMoEFlattenIntraCycleMoELayer(configs, self.temperature_experts+self.anode_experts)]),
        ...
        self.intra_MoE_layers = nn.ModuleList([MultiViewLayer(self.d_model, self.d_model,
                                                    specification_view_experts=nn.ModuleList([BatteryMoEIntraCycleMoELayer(configs, self.cathode_experts),
                                                                                            BatteryMoEIntraCycleMoELayer(configs, self.anode_experts)]),
                                                    operating_view_experts=nn.ModuleList([BatteryMoEIntraCycleMoELayer(configs, self.temperature_experts+self.anode_experts)]),
                                                    norm_layer=nn.LayerNorm(self.d_model),
                                                    general_experts=nn.ModuleList([
        ...
        self.inter_MoE_layers = nn.ModuleList([MultiViewTransformerLayer(self.d_model, self.d_model, self.n_heads,
                                                    specification_view_experts=nn.ModuleList([BatteryMoEInterCycleMoELayer(configs, self.cathode_experts),
                                                                                            BatteryMoEInterCycleMoELayer(configs, self.anode_experts)]), 
                                                    operating_view_experts=nn.ModuleList([BatteryMoEInterCycleMoELayer(configs, self.temperature_experts+self.anode_experts)]),
                                                    general_experts=nn.ModuleList([
                                                        MLPBlockGELU(self.d_model, self.d_ff, self.drop_rate, self.activation) for _ in range(self.num_general_experts)
                                                    ]),
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        params:
            ...
            xxx_masks: [B, num_experts]. 0 indicates masked
        '''
        ...
        total_masks = [cathode_masks, anode_masks, torch.cat([temperature_masks, format_masks], dim=1)]
        ...
        operating_logits = logits[:, :, self.cathode_experts+self.anode_experts:]
        ...
        out, guide_loss = self.flattenIntraCycleLayer(self.flattn(cycle_curve_data), [cathode_logits[:,logits_index], anode_logits[:,logits_index], operating_logits[:,logits_index]], total_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        logits_index += 1

        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss = intra_MoELayer(out, [cathode_logits[:,logits_index], anode_logits[:,logits_index], operating_logits[:,logits_index]], total_masks) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1
        ...
        for i, inter_MoELayer in enumerate(self.inter_MoE_layers):
            out, guide_loss = inter_MoELayer(out, [cathode_logits[:,logits_index], anode_logits[:,logits_index], operating_logits[:,logits_index]], total_masks, attn_mask=attn_mask) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1

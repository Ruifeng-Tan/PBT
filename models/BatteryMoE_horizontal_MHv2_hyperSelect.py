'''
基于BatteryMoE_horizontal_MHv2，只不过基于aging condition做feature selection
'''
import torch
...
class Model(nn.Module):
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        ac_logits = self.aging_condition_act(DKP_embeddings)
        _, indices = torch.topk(ac_logits, self.low_d_ff, dim=1)
        mask = torch.zeros_like(ac_logits, dtype=torch.bool)

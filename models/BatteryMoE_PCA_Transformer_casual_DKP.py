'''
基于BatteryMoE_PCA_Transformer_Imp，只不过将attention mask改成了casual attention mask，此外将DKP Embedding作为了regression token加在了序列的最后面
'''
import torch
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.reg_token_MLP = nn.Sequential(nn.Linear(self.d_llm, self.d_ff), nn.ReLU(), nn.Dropout(self.drop_rate), nn.Linear(self.d_ff, self.d_model))
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        reg_token = self.reg_token_MLP(DKP_embeddings).unsqueeze(1)
        reg_attn_mask = torch.ones_like(curve_attn_mask[:,:1]) # [B, 1]
        tmp_mask = torch.cat([curve_attn_mask, reg_attn_mask], dim=1) # [B, L+1]

        out = out + self.pe(out) # add positional encoding
        out = torch.cat([out, reg_token], dim=1)
        attn_mask = tmp_mask.unsqueeze(1) # [B, 1, L]
        attn_mask = torch.repeat_interleave(attn_mask, attn_mask.shape[-1], dim=1) # [B, L, L]
        casual_attn_mask = self.create_causal_mask(B, attn_mask.shape[1], device=attn_mask.device) # [B, L, L]
        attn_mask = torch.where(casual_attn_mask==1, attn_mask, casual_attn_mask) # convert the attention mask to casual attn mask and the invalid tokens are masked
    ...
    def create_causal_mask(self, B, seq_len, device):
        '''
        return:
            casual mask: [B, L, L]. 0 indicates masked.
        '''
        # Create a lower triangular matrix of shape (seq_len, seq_len)
        mask = torch.tril(torch.ones(seq_len, seq_len, device=device))  # (L, L)

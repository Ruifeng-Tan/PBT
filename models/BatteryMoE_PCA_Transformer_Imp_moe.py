'''
基于BatteryMoE_PCA_Transformer，只不过Outhead只取最后一个valid token做回归任务，而且CyclePatch层不再做MoE
'''
import torch
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.moe_layers = configs.d_layers
        self.drop_rate = configs.dropout
        self.activation = configs.activation
        ...
        self.flattenIntraCycleLayer = nn.Sequential(nn.Flatten(start_dim=2), nn.Linear(self.charge_discharge_length*3, self.d_model))
        
        self.intra_MoE_layers = nn.ModuleList([MLPBlockGELU(self.d_model, self.d_ff, self.drop_rate, self.activation) for _ in range(self.e_layers)])
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out = intra_MoELayer(out) # [B, L, d_model]
            # total_guide_loss += guide_loss
            # total_aug_count += 1
            # logits_index += 1

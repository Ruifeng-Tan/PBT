...
from layers.distributional_router_encoder import PatternRouter_two, PatternRouter
...
class IntraCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.noisy_gating = configs.noisy_gating
        self.gate  = PatternRouter_two(self.d_llm, self.d_model, self.d_ff, self.num_experts)
        self.noise = PatternRouter_two(self.d_llm, self.d_model, self.d_ff, self.num_experts)
        self.softplus = nn.Softplus()
        self.eps = 1e-9
    ...
    def forward(self, cycle_curve_data, DKP_embeddings):
        ...
        DKP_embeddings = torch.repeat_interleave(DKP_embeddings, dim=0, repeats=L) # [B*L, d_llm]
        cycle_curve_data = cycle_curve_data.reshape(B*L, -1)
        logits = self.gate(DKP_embeddings, cycle_curve_data)
        ...
        # Scatter the mask at the indices of the top-K values
        mask.scatter_(1, indices, 1) # 0 indicates mask
        logits = F.softmax(logits, dim=1) # [B*L, num_experts]
        raw_logits = logits.clone()
        # logits.masked_fill_(mask==0, 0) # [B*L, num_experts]
        logits = logits * mask
        de_norm = torch.sum(logits, dim=1) + self.eps
        logits = logits / de_norm.unsqueeze(-1)

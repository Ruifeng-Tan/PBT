...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.gate = nn.Sequential(nn.Linear(self.d_llm, (self.cathode_experts + self.temperature_experts + self.format_experts)*2+self.cathode_experts+self.cathode_experts))
        self.flattenIntraCycleLayer = CathodeFlattenIntraCycleMoELayer(configs)
        self.intra_CathodeMoE = HEIntraCycleMoELayer(configs, configs.cathode_experts)
        ...
        self.inter_CathodeMoE = HEInterCycleMoELayer(configs, configs.cathode_experts)
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        out, _, guide_loss = self.intra_CathodeMoE(out, cathode_logits[:,:self.cathode_experts], cathode_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        ...
        out, _, guide_loss = self.inter_CathodeMoE(out, cathode_logits[:,self.cathode_experts:], cathode_masks) # [B, L, d_model]
        total_guide_loss += guide_loss

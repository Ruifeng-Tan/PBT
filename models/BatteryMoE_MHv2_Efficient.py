'''
基于BatteryMoE_MHv2_PESum, 只不过在每个view内做rescale和recenter
'''
import torch
...
class PESumLinear(nn.Module):
    ...
    def forward(self, x, curve_attn_mask):
        ...
        act_x = act_x * curve_attn_mask.unsqueeze(-1) # [B, L, D]
        out = torch.sum(act_x, dim=1)
        return out

class PECyclePatchLayer(nn.Module):
    '''
    To improve the parameter efficient of the original CPMLP regarding the CyclePatch embedding,
    this module is introduced.
    '''
    def __init__(self, configs):
        super(PECyclePatchLayer, self).__init__()
        self.d_model = configs.d_model
        self.charge_discharge_length = configs.charge_discharge_length
        self.W = nn.Linear(self.charge_discharge_length, self.d_model*2//3, bias=False)
        self.act = nn.Sigmoid()
        self.V = nn.Linear(self.charge_discharge_length, self.d_model*2//3, bias=False)
        self.W2 = nn.Linear(self.d_model*2//3, self.d_model, bias=False)
    
    def forward(self, x):
        '''
        x: [B, L, sampling_points, num_var]
        '''
        swish_x = self.W(x)
        act_x = self.W2(swish_x * self.act(swish_x) * self.V(x))
        out = torch.sum(act_x, dim=2) # [B, L, D]
        return out
...
class MultiViewLayer(nn.Module):
    def __init__(self, d_model, num_views, view_experts, norm_layer, general_experts, use_connection, use_general_norm=True):
        ...
        self.use_general_norm =use_general_norm
        if use_general_norm:
            self.general_norm = DomainKnowledgeViewAffine(d_model)
    ...
    def forward(self, x, total_logits, total_masks, curve_attn_mask=None):
        ...
        for i, view_expert in enumerate(self.view_experts):
            if curve_attn_mask is None:
                out, guide_loss, aug_loss = view_expert(x, total_logits[i], total_masks[i])
            else:
                out, guide_loss, aug_loss = view_expert(x, total_logits[i], total_masks[i], curve_attn_mask=curve_attn_mask)
        ...
        general_x = self.general_norm(x) if self.use_general_norm else x
        for i in range(len(self.general_experts)):
            if curve_attn_mask is None:
                final_out = self.general_experts[i](general_x) + final_out # add the general experts
            else:
                final_out = self.general_experts[i](general_x, curve_attn_mask) + final_out
...
class BatteryMoEFlattenIntraCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts, topK=None, use_aug_loss=False):
        ...
        self.experts = nn.ModuleList([PECyclePatchLayer(configs) for _ in range(self.num_experts)])
...
class BatteryMoEIntraCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts, topK=None, use_aug_loss=False):
        ...
        self.view_norm = DomainKnowledgeViewAffine(self.d_model)
    ...
    def forward(self, cycle_curve_data, logits, moe_masks):
        ...
        '''
        B = cycle_curve_data.shape[0]
        cycle_curve_data = self.view_norm(cycle_curve_data)
...
class BatteryMoEFlattenInterCycleMoELayer(nn.Module):
    ...
    def forward(self, cycle_curve_data, logits, moe_masks, curve_attn_mask=None):
        '''
        params:
            cycle_curve_data: [B, L, d_model]
            logits: [B, num_experts]
        ...
        total_expert_outs = []
        for i, expert in enumerate(self.experts):
            out = expert(cycle_curve_data[MOE_indicies[i]], curve_attn_mask[MOE_indicies[i]]) # [expert_batch_size, d_model]
            total_outs.append(out)
            if len(MOE_indicies[i])>=1:
                total_expert_outs.append(out)
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.flattenIntraCycleLayer = MultiViewLayer(self.d_model, self.num_views, 
                                                     nn.ModuleList([BatteryMoEFlattenIntraCycleMoELayer(configs, self.cathode_experts),
                                                                    ...
                                                                    BatteryMoEFlattenIntraCycleMoELayer(configs, self.temperature_experts),
                                                                    BatteryMoEFlattenIntraCycleMoELayer(configs, self.format_experts)],
                                                                    ),
                                                    norm_layer=nn.LayerNorm(self.d_model, elementwise_affine=False),
                                                    general_experts=nn.ModuleList([
                                                        PECyclePatchLayer(configs) for _ in range(self.num_general_experts)
                                                    ]),
                                                    use_connection=False, use_general_norm=False)
        
        self.intra_MoE_layers = nn.ModuleList([MultiViewLayer(self.d_model, self.num_views, 
                                                     nn.ModuleList([BatteryMoEIntraCycleMoELayer(configs, self.cathode_experts),
                                                                    BatteryMoEIntraCycleMoELayer(configs, self.anode_experts),
                                                                    BatteryMoEIntraCycleMoELayer(configs, self.temperature_experts),
                                                                    BatteryMoEIntraCycleMoELayer(configs, self.format_experts)
                                                    ]),
                                                    norm_layer=nn.LayerNorm(self.d_model, elementwise_affine=False),
                                                    general_experts=nn.ModuleList([
        ...
        self.flattenInterCycleLayer = MultiViewLayer(self.d_model, self.num_views, 
                                                     nn.ModuleList([BatteryMoEFlattenInterCycleMoELayer(configs, self.cathode_experts),
                                                                    BatteryMoEFlattenInterCycleMoELayer(configs, self.anode_experts),
                                                                    BatteryMoEFlattenInterCycleMoELayer(configs, self.temperature_experts),
                                                                    BatteryMoEFlattenInterCycleMoELayer(configs, self.format_experts)
                                                    ]),
                                                    norm_layer=nn.LayerNorm(self.d_model, elementwise_affine=False),
                                                    general_experts=nn.ModuleList([
        ...
        self.inter_MoE_layers = nn.ModuleList([MultiViewLayer(self.d_model, self.num_views, 
                                                     nn.ModuleList([BatteryMoEInterCycleMoELayer(configs, self.cathode_experts),
                                                                    BatteryMoEInterCycleMoELayer(configs, self.anode_experts),
                                                                    BatteryMoEInterCycleMoELayer(configs, self.temperature_experts),
                                             ...
                                             + 
                                             [MultiViewLayer(self.d_model, self.num_views, 
                                                nn.ModuleList([BatteryMoEInterCycleMoELayer(configs, self.cathode_experts),
                                                            BatteryMoEInterCycleMoELayer(configs, self.anode_experts),
                                                            BatteryMoEInterCycleMoELayer(configs, self.temperature_experts),
                                                            BatteryMoEInterCycleMoELayer(configs, self.format_experts)
                                            ]), norm_layer=nn.LayerNorm(self.d_model, elementwise_affine=True),
                                            general_experts=nn.ModuleList([
                                                MLPBlockGELU(self.d_model, self.d_ff, self.drop_rate, self.activation) for _ in range(self.num_general_experts)
                                            ]),
                                            use_connection=True
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        out, guide_loss, aug_loss = self.flattenInterCycleLayer(out, [cathode_logits[:,logits_index],anode_logits[:,logits_index],temperature_logits[:,logits_index],format_logits[:,logits_index]], total_masks, curve_attn_mask=curve_attn_mask)
        total_guide_loss += guide_loss
        total_aug_loss += aug_loss
        total_aug_count += 1
        logits_index += 1

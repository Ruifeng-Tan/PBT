'''
基于BatteryMoE_Hyper_CropAugIMP，只不过在数据增强的时候通过均匀降低频率再插值
...
class Model(nn.Module):
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        if use_aug:
            down_sample_step = np.random.randint(2, 4)
            ...
            flatten_cycle_curve_data[B:] = self.CD_Crop_augmentation(flatten_cycle_curve_data[B:], down_sample_step) # add noise
    ...
    def CD_Crop_augmentation(self, X: torch.Tensor, down_sample_step: int) -> torch.Tensor:
        """
        ...
        Args:
            ...
            down_sample_step (int): The down sampling step.
        ...
        charge_X = X[:, :L//2]
        discharge_X = X[:, L//2:]
        aug_charge_X = self.Crop(charge_X, down_sample_step=down_sample_step)
        aug_discharge_X = self.Crop(discharge_X, down_sample_step=down_sample_step)
    ...
    def Crop(self, X: torch.Tensor, down_sample_step: int) -> torch.Tensor:
        # X shape: [B, L, D]
        B, L, D = X.shape
        device = X.device
        
        # Generate initial indices with down_sample_step
        indices = torch.arange(0, L, down_sample_step, device=device)
        
        # Ensure the last index is L-1
        if indices[-1] != L - 1:
            indices = torch.cat([indices, torch.tensor([L-1], device=device)])
        
        # Downsample the tensor
        downsampled_X = X[:, indices, :]  # Shape [B, K, D]
        
        # Permute dimensions for interpolation: [B, D, K]
        downsampled_X = downsampled_X.permute(0, 2, 1)
        
        # Linear interpolation to original length L
        ...
        # Permute back to original shape [B, L, D]
        interpolated_X = interpolated_X.permute(0, 2, 1)

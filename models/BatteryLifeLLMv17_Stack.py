...
from layers.distributional_router_encoder import DistributionRouter, PatternRouter_Imp, PatternRouter_Imp_three
...
class FlattenIntraCycleMoELayer(nn.Module):
    def __init__(self, configs):
        ...
        self.gate  = PatternRouter_Imp(self.d_llm, 1, self.d_ff, self.num_experts)
        self.noise = PatternRouter_Imp(self.d_llm, 1, self.d_ff, self.num_experts)
        self.softplus = nn.Softplus()
        self.eps = 1e-9

    
    def forward(self, cycle_curve_data, cylce_numbers, DKP_embeddings):
        '''
        params:
        ...
        clean_logits = self.gate(DKP_embeddings, cylce_numbers)
        if self.noisy_gating and self.training:
            raw_noise_stddev = self.noise(DKP_embeddings, cylce_numbers)

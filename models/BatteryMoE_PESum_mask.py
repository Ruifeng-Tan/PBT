'''
基于BatteryMoE_DKPNorm，只不过用来Positional Encoding和GEGLU的形式来提升FlattenInterCycle的参数效率，只不过使用LayerNorm
'''
import torch
...
class Model(nn.Module):
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        out, guide_loss, aug_loss = self.flattenInterCycleLayer(out, [logits[:,logits_index]], total_masks, curve_attn_mask=curve_attn_mask)
        total_guide_loss += guide_loss
        total_aug_loss += aug_loss
        total_aug_count += 1
        logits_index += 1

...
from layers.SelfAttention_Family import FullAttention_CustomDropout, AttentionLayer
...
class MultiViewTransformerLayer(nn.Module):
    def __init__(self, d_model, n_heads, view_experts, general_experts, drop_rate):
        ...
        self.attention = AttentionLayer(FullAttention_CustomDropout(True, 1, attention_dropout=drop_rate,
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.DKP_proj = nn.Sequential(nn.Linear(self.d_llm, self.d_ff), nn.ReLU(), nn.Dropout(self.drop_rate),
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        out = out + self.pe(out) + self.DKP_proj(DKP_embeddings).unsqueeze(1) # add positional encoding and DKP embeddings

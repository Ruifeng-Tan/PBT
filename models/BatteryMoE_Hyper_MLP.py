'''
基于BatteryMoE_Hyper_CropAugIMP，只不过使用一个三层的MLP作为Head
...
class OutputHead(nn.Module):
    def __init__(self, ec_config):
        ...
        self.projection = nn.Sequential(nn.Linear(self.d_model, self.d_model//2), nn.ReLU(),
                                        nn.Linear(self.d_model//2, self.d_model), nn.ReLU(),
                                        nn.Linear(self.d_model, ec_config.output_num))

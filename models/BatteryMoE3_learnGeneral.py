...
class Model(nn.Module):
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        # if torch.any(torch.isnan(cathode_masks)):
        #     raise Exception('Nan in mask')
        # if torch.any(torch.isnan(temperature_masks)):
        #     raise Exception('Nan in mask')
        # if torch.any(torch.isnan(format_masks)):
        #     raise Exception('Nan in mask')
  
        cathode_logits = logits[:, :self.cathode_experts*2]
        temperature_logits = logits[:, self.cathode_experts*2:self.cathode_experts*2+self.temperature_experts*2]
        format_logits = logits[:, self.cathode_experts*2+self.temperature_experts*2:]
        # logits_index = 0
        ...
        out, _, guide_loss = self.flattenIntraCycleLayer(cycle_curve_data, cathode_logits[:, :self.cathode_experts], cathode_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        # logits_index += 1

        out, _, guide_loss = self.intra_TemperatureMoE(out, temperature_logits[:, :self.temperature_experts], temperature_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        # logits_index += 1

        out, _, guide_loss = self.intra_FormatMoE(out, format_logits[:, :self.format_experts], format_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        # logits_index += 1
        
        out, _, guide_loss = self.flattenInterCycleLayer(out, cathode_logits[:, self.cathode_experts:], cathode_masks)
        total_guide_loss += guide_loss
        total_aug_count += 1
        # logits_index += 1


        out, _, guide_loss = self.inter_TemperatureMoE(out, temperature_logits[:, self.temperature_experts:], temperature_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        # logits_index += 1

        out, _, guide_loss = self.inter_FormatMoE(out, format_logits[:, self.format_experts:], format_masks) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_aug_count += 1
        # logits_index += 1

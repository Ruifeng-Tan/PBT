...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.intra_cycle_BiGRU = nn.GRU(configs.d_model, configs.d_ff, configs.lstm_layers, batch_first=True, dropout=self.drop_rate, bidirectional=True)
        self.norm = nn.LayerNorm(configs.d_ff*2)
        self.inter_cycle_BiGRU = nn.GRU(configs.d_ff*2, configs.d_ff, configs.lstm_layers, batch_first=True, dropout=self.drop_rate, bidirectional=True)
    ...
    def classification(self, x_enc, curve_attn_mask):
        ...
        x_enc, _ = self.intra_cycle_BiGRU(x_enc)

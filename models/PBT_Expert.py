...
import json
from layers.MLPs import BatteryMoEMLP, FlattenLinear
transformers.logging.set_verbosity_error() 

class MultiViewLayer(nn.Module):
    def __init__(self, gate_input_dim, num_experts, view_experts, norm_layer, general_experts, use_connection, use_norm=True):
    ...
    def forward(self, x, gate_input, total_masks, use_view_experts):
        '''
        x: [N, *, in_dim]
        ...
        if use_view_experts:
            for i, view_expert in enumerate(self.view_experts):
                out, guide_loss, LB_loss = view_expert(x, total_logits, total_masks[i], gate_input)
                final_out = final_out + out
                total_guide_loss += guide_loss
                total_LB_loss += LB_loss

        for i in range(len(self.general_experts)):
            final_out = self.general_experts[i](x, gate_input) + final_out # add the general experts
...
class MultiViewTransformerLayer(nn.Module):
    ...
    def forward(self, x, gate_input, total_masks, attn_mask, use_view_experts):
        '''
        x: [N, *, in_dim]
        gate_input: [B, gate_input_dim]
        total_masks: [num_view, num_experts for each view expert]
...
class BatteryMoEFlattenIntraCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts, gate_d_ff):
        ...
        self.experts = nn.ModuleList([FlattenLinear(gate_d_ff, self.charge_discharge_length*3, self.d_model) for _ in range(self.num_experts)])
        self.eps = 1e-9
    
    def forward(self, cycle_curve_data, logits, moe_masks, gate_input):
        '''
        params:
        ...
        for i, expert in enumerate(self.experts):
            if len(MOE_indicies[i])>=1:
                out = expert(cycle_curve_data[MOE_indicies[i]], gate_input[MOE_indicies[i]]) # [expert_batch_size, d_llm]
...
class BatteryMoEIntraCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts, d_ff_scale_factor, gate_d_ff):
        ...
        self.experts = nn.ModuleList([BatteryMoEMLP(gate_d_ff, self.d_model, math.ceil(self.d_ff * d_ff_scale_factor[i]), self.drop_rate, self.activation) for i in range(self.num_experts)])
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.flattenIntraCycleLayer = MultiViewLayer(gate_input_dim, self.num_experts,
                                                     nn.ModuleList([BatteryMoEFlattenIntraCycleMoELayer(configs, self.num_experts, self.gate_d_ff)]
                                                                    ),
                                                    norm_layer=nn.LayerNorm(self.d_model),
                                                    general_experts=nn.ModuleList([
                                                        FlattenLinear(self.gate_d_ff, self.charge_discharge_length*3, self.d_model) for _ in range(self.num_general_experts)
                                                    ]),
                                                    use_connection=False, use_norm=False)
        
        self.intra_MoE_layers = nn.ModuleList([MultiViewLayer(gate_input_dim, self.num_experts,
                                                     nn.ModuleList([BatteryMoEIntraCycleMoELayer(configs, self.num_experts, self.d_ff_scale_factor, self.gate_d_ff)
                                                    ]),
                                                    norm_layer=nn.LayerNorm(self.d_model),
                                                    general_experts=nn.ModuleList([
                                                        BatteryMoEMLP(self.gate_d_ff, self.d_model, self.d_ff, self.drop_rate, self.activation) for _ in range(self.num_general_experts)
                                                    ]),
        ...
        self.inter_MoE_layers = nn.ModuleList([MultiViewTransformerLayer(gate_input_dim, self.num_experts,self.d_model, self.n_heads,
                                                     nn.ModuleList([BatteryMoEInterCycleMoELayer(configs, self.num_experts, self.d_ff_scale_factor, self.gate_d_ff),
                                                    ]), 
                                                    general_experts=nn.ModuleList([
                                                        BatteryMoEMLP(self.gate_d_ff, self.d_model, self.d_ff, self.drop_rate, self.activation) for _ in range(self.num_general_experts)
                                                    ]),
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        out, guide_loss, LB_loss = self.flattenIntraCycleLayer(cycle_curve_data, DKP_embeddings, total_masks, use_view_experts=use_view_experts) # [B, L, d_model]
        total_guide_loss += guide_loss
        total_LB_loss += LB_loss
        total_aug_count += 1
        logits_index += 1
        ...
        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss, LB_loss = intra_MoELayer(out, DKP_embeddings, total_masks, use_view_experts=use_view_experts) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_LB_loss += LB_loss
            total_aug_count += 1
            logits_index += 1
        ...
        for i, inter_MoELayer in enumerate(self.inter_MoE_layers):
            out, guide_loss, LB_loss = inter_MoELayer(out, DKP_embeddings, total_masks, attn_mask=attn_mask, use_view_experts=use_view_experts) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_LB_loss += LB_loss
            total_aug_count += 1
            logits_index += 1

...
class InterCycleFlattenLayer(nn.Module):
    def __init__(self, configs):
        super(InterCycleFlattenLayer, self).__init__()
        self.bottleneck_factor = configs.bottleneck_factor
        self.d_model = configs.d_model
        self.linear = nn.Linear(self.d_model, self.d_model)
        self.linear2 = nn.Sequential(nn.Linear(configs.early_cycle_threshold, self.d_model), 
    ...
    def forward(self, x):
        ...
        out = self.linear2(x).squeeze(-1) # [B, d_model]
        return out

class MultiViewLayer(nn.Module):
    def __init__(self, num_views, view_experts, norm_layer, general_experts, use_connection, use_norm):
    ...
    def forward(self, x, total_logits, total_masks, DKP_embeddings=None):
        ...
        if self.use_connection:
            ...
            final_out = self.norm(final_out, DKP_embeddings) # add & norm
...
class BatteryMoEFlattenInterCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts, topK=None, use_aug_loss=False):
        ...
        self.activation = configs.activation
        self.top_k = topK if topK is not None else configs.topK
        self.experts = nn.ModuleList([InterCycleFlattenLayer(configs) for _ in range(self.num_experts)])
        self.num_general_experts = configs.num_general_experts
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.intra_MoE_layers = nn.ModuleList([MultiViewLayer(self.num_views, 
                                                    ...
                                                    use_connection=True, use_norm=True) for _ in range(self.e_layers)])
        ...
        self.flattenInterCycleLayer = MultiViewLayer(self.num_views, 
                                                    ...
                                                    norm_layer=DKINorm(self.d_model, self.d_llm, self.drop_rate),
                                                    general_experts=nn.ModuleList([
                                                        InterCycleFlattenLayer(configs) for _ in range(self.num_general_experts)
                                                    ]),
        ...
        self.inter_MoE_layers = nn.ModuleList([MultiViewLayer(self.num_views, 
                                                    ...
                                                    use_connection=True, use_norm=True
                                                    )
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss, aug_loss = intra_MoELayer(out, [logits[:,logits_index]], total_masks, DKP_embeddings) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_loss += aug_loss
            total_aug_count += 1
            logits_index += 1
        ...
        for i, inter_MoELayer in enumerate(self.inter_MoE_layers):
            out, guide_loss, aug_loss = inter_MoELayer(out, [logits[:,logits_index]], total_masks, DKP_embeddings)
            total_guide_loss += guide_loss
            total_aug_loss += aug_loss
            total_aug_count += 1
            logits_index += 1

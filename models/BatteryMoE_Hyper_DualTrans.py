'''
基于BatteryMoE_Hyper_CropAugIMP，只不过将CyclePatch也改成了基于Transformer的结构
...
from layers.Embed import TokenEmbedding, DataEmbedding, ChannelMixingPatchEmbedding
...
class PatchLinear(nn.Module):
    '''
    Used in the CyclePatch
    '''
    def __init__(self, embed_dim, dropout):
        super(PatchLinear, self).__init__()
        self.embed_dim = embed_dim
        self.patch_embedding = ChannelMixingPatchEmbedding(
                    embed_dim, 150, 10, 10, dropout)
        
    def forward(self, x):
        '''
        params:
            x: [B, n_vars, fixed_len]
        '''
        x, _ = self.patch_embedding(x) # [B, len_after_patch, D]
        return x
...
class MultiViewLayer(nn.Module):
    ...
    def forward(self, x, total_logits, total_masks, selection_embeddings):
        '''
        x: [B, L, num_vars, fixed_length_of_curve]
        ...
        '''
        B, L, fixed_len = x.shape[0], x.shape[1], x.shape[3]
...
class IntraMultiViewTransformerLayer(nn.Module):
    def __init__(self, hyperMoEs, low_d_ff, d_model, n_heads, view_experts, general_experts, drop_rate):
        super(IntraMultiViewTransformerLayer, self).__init__()
        self.num_views = len(view_experts)
        self.hyperMoEs = hyperMoEs
        self.layer_embedding = nn.Parameter(torch.empty(1, low_d_ff // 2))
    ...
    def forward(self, x, total_logits, total_masks, attn_mask, selection_embeddings):
        '''
        x: [N, L, n_patchs, in_dim]. The input shape is different from the input shape of MultiViewTransformerLayer
        total_logits: [num_view, num_experts for each view expert]
        total_masks: [num_view, num_experts for each view expert]
        attn_mask: [B, 1, L, L]
        selection_embeddings: [B, num_experts, low_d_ff // 2]
        '''
        B, L, fixed_len = x.shape[0], x.shape[1], x.shape[2]
        layer_embedding = self.layer_embedding.expand(B, -1)
        # casual masked self-attention
        x = x.reshape(B*L, fixed_len, -1)
        ...
        x = self.norm1(x)
        x = x.reshape(B, L, fixed_len, -1) # reshape back
...
class BatteryMoEFlattenIntraCycleMoELayer(nn.Module):
    def __init__(self, configs, num_experts):
        ...
        self.experts = nn.ModuleList([nn.Sequential(PatchLinear(self.d_model, self.drop_rate)) for _ in range(self.num_experts)])
        self.eps = 1e-9
    
    def forward(self, cycle_curve_data, logits, moe_masks, selection_embeddings):
        '''
        params:
            cycle_curve_data: [B, L, num_vars, fixed_length_of_curve]
            DKP_embeddings: [B, num_experts]
            moe_masks: [B, num_experts]
            selection_embeddings: [B, num_experts, low_d_ff // 2]
        return:
            final_out: [B*L, n_patchs, d_model]
        '''
        ...
        for i, expert in enumerate(self.experts):
            if len(MOE_indicies[i])>=1:
                out = expert(cycle_curve_data[MOE_indicies[i]]) # [expert_batch_size, L, num_vars, fixed_length_of_curve]
                print(out.shape)
                total_outs.append(out)
                total_expert_outs.append(out)


        total_outs = dispatcher.combine(total_outs).to(torch.bfloat16) # [B, L*n_paths, d_model]
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.flattenIntraCycleLayer = MultiViewLayer([], self.low_d_ff,
                                                     nn.ModuleList([BatteryMoEFlattenIntraCycleMoELayer(configs, self.num_experts)]
                                                                    ),
                                                    norm_layer=nn.LayerNorm(self.d_model),
                                                    general_experts=nn.ModuleList([
                                                        nn.Sequential(PatchLinear(self.d_model, self.drop_rate)) for _ in range(self.num_general_experts)
                                                    ]),
                                                    use_connection=False)
        
        self.pe = PositionalEmbedding(self.d_model)
        self.intra_MoE_layers = nn.ModuleList([IntraMultiViewTransformerLayer(self.shared_hyperMoE, self.low_d_ff, self.d_model, self.n_heads,
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, 
        ...
        # Intra-cycle modelling using Transformer with MoE FFN
        out = out.reshape(B*L, -1, self.d_model)
        out = out + self.pe(out) # [B*L, n_patchs, D] 
        for i, intra_MoELayer in enumerate(self.intra_MoE_layers):
            out, guide_loss = intra_MoELayer(out, [logits[:,logits_index]], total_masks, attn_mask=None, selection_embeddings=selection_embeddings) # [B, L, d_model]
            total_guide_loss += guide_loss
            total_aug_count += 1
            logits_index += 1

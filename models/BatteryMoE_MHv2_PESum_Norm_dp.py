...
class MultiViewLayer(nn.Module):
    def __init__(self, dropout, d_model, num_views, view_experts, norm_layer, general_experts, use_connection, use_general_norm=True):
    ...
    def forward(self, x, total_logits, total_masks, curve_attn_mask=None):
        ...
        general_x = self.dropout(general_x)
...
class BatteryMoEIntraCycleMoELayer(nn.Module):
    ...
    def forward(self, cycle_curve_data, logits, moe_masks):
        ...
        cycle_curve_data = self.dropout(cycle_curve_data)
...
class Model(nn.Module):
    ...
    def __init__(self, battery_life_config):
        ...
        self.flattenIntraCycleLayer = MultiViewLayer(self.drop_rate, self.d_model, self.num_views, 
        ...
        self.intra_MoE_layers = nn.ModuleList([MultiViewLayer(self.drop_rate, self.d_model, self.num_views, 
        ...
        self.flattenInterCycleLayer = MultiViewLayer(self.drop_rate, self.d_model, self.num_views, 
        ...
        self.inter_MoE_layers = nn.ModuleList([MultiViewLayer(self.drop_rate, self.d_model, self.num_views, 
                                             ...
                                             + 
                                             [MultiViewLayer(self.drop_rate, self.d_model, self.num_views, 
                                                nn.ModuleList([BatteryMoEInterCycleMoELayer(configs, self.cathode_experts),
                                                            BatteryMoEInterCycleMoELayer(configs, self.anode_experts),
                                                            BatteryMoEInterCycleMoELayer(configs, self.temperature_experts),
                                                            BatteryMoEInterCycleMoELayer(configs, self.format_experts)

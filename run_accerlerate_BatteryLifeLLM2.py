...
from models import BatteryMoE_Gating_clv2, BatteryMoE_Gating_cl, BatteryMoE_baseline, BatteryMoE_baseline_Gating, BatteryMoE_v1_casual, BatteryMoE_Gating_dVdQ_multi, BatteryMoE_Gating
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_Gating_clv2':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_Gating_clv2.Model(model_config)
    else:

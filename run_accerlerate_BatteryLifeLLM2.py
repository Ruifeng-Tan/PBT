...
from models import BatteryMoE_Gating_ImpGate, BatteryMoE_Gating_Decompose, BatteryMoE_baseline, BatteryMoE_baseline_Gating, BatteryMoE_Gating_Compressed, BatteryMoE_Gating_dVdQ_multi, BatteryMoE_Gating
import wandb
...
for ii in range(args.itr):
    ...
    elif args.model == 'BatteryMoE_Gating_Decompose':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_Gating_Decompose.Model(model_config)
    ...
    elif args.model == 'BatteryMoE_Gating_Compressed':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_Gating_Compressed.Model(model_config)
    elif args.model == 'BatteryMoE_baseline':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_baseline.Model(model_config)
    elif args.model == 'BatteryMoE_Gating_ImpGate':
        model_ec_config = BatteryElectrochemicalConfig(args.__dict__)
        model_text_config = AutoConfig.from_pretrained(args.LLM_path)
        model_config = BatteryLifeConfig(model_ec_config, model_text_config)
        model = BatteryMoE_Gating_ImpGate.Model(model_config)
    else:
